...
6000, 800, step=50)
 if preset != "Custom":
     ...
     eur_por_1k_tokens = st.number_input("Coste modelo (€/1k tokens)", min_value=0.0, value=0.80, step=0.05)
 fcr = st.slider("Calidad (FCR estimado)", 0.10, 0.95, 0.72,
step=0.01)
 coste_hora_agente = st.number_input("Coste hora agente humano (€)", min_value=0.0, value=25.0, step=1.0)
...
if fcr < 0.65 and r["coste_operativo"] > r["coste_ti"]:
 st.error("⚠️ ALERTA: estás ahorrando en TI, pero el retrabajo humano domina el coste total (SI).")
elif r["coste_operativo"] > r["coste_ti"]:
 st.warning("Atención: el coste operativo ya supera el coste TI. Revisa FCR/HITL/guardrails.")
else:
 st.success("Buen balance: el coste TI domina (o está equilibrado) y el retrabajo está contenido.")

st.caption("Interpretación: los tickets que la IA no resuelve (1-FCR) los paga el humano (AHT * €/hora).")
